 ...
 * ```
 */
export function parse(address: string): ResultUtils<Octets, ParseError> {
  ...
  for (let i = 0; i < octets.length; i++) {
    ...
    const num = Number.parseInt(octet as "");
...
export function validate(address: IPv4Address): boolean {
  ...
  for (let i = 0; i < address.length; i++) {
    ...
    if (!octet || typeof octet === undefined) return false;

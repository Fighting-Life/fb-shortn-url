...
type _ResultUtils<T, E> = { ok: true; val: T } | { ok: false; err: E };
 ...
 * ```
 */
class ResultUtils<T, E> {
  private readonly _result: _ResultUtils<T, E>;

  constructor(result: _ResultUtils<T, E>) {
    this._result = result;
  }
   ...
   * ```
   */
  map<A>(fn: (val: T) => A): ResultUtils<A, E> {
    return this.match(
      (val) => Ok(fn(val)),
      (err) => Err(err),
    );
   ...
   * ```
   */
  mapErr<A>(fn: (err: E) => A): ResultUtils<T, A> {
    return this.match(
      (val) => Ok(val),
      (err) => Err(fn(err)),
    );
 ...
 * ```
 */
export function Ok<T>(val: T): ResultUtils<T, never> {
  return new ResultUtils<T, never>({ ok: true, val });
}
 ...
 * ```
 */
export function Err<E>(err: E): ResultUtils<never, E> {
  return new ResultUtils<never, E>({ ok: false, err });
}

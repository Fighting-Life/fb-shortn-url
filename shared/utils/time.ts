...
export function formatToPostgresTimestamp(date: Date): string {
  ...
  const milliseconds =
    String(date.getUTCMilliseconds()).padStart(3, "0") + "000";
...
export function convertDayToDateFormatYYYYMMDD(
  dayOfWeek: string,
): string | null {
  const days = [
    "Sunday",
...
export function convertDayStringToDateYYYYMMDD(
  ...
  const date = new Date(year || 0, month || 0, day || 0 - 1);
...
export function getDateForDayYYYYMMDD(
  dayName: string,
  weekOffset: number = 0,
): string {
...
export function parseSecondToTimeComponents(
  totalSeconds: number,
): TimeComponents {
  const hours = Math.floor(totalSeconds / 3600);
  const minutes = Math.floor((totalSeconds % 3600) / 60);
